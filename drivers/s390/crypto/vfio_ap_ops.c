...
static void vfio_ap_free_aqic_resources(struct vfio_ap_queue *q)
{
	...
	if (q->saved_pfn && !WARN_ON(!q->matrix_mdev)) {
		vfio_unpin_pages(q->matrix_mdev->vdev.dev,
				 &q->saved_pfn, 1);
		q->saved_pfn = 0;
	}
}
						 ...
						 unsigned long nib)
{
	...
	ret = vfio_pin_pages(q->matrix_mdev->vdev.dev, &g_pfn, 1,
	...
	case AP_RESPONSE_OTHERWISE_CHANGED:
		...
		vfio_unpin_pages(q->matrix_mdev->vdev.dev, &g_pfn, 1);
				       ...
				       unsigned long action, void *data)
{
	...
	if (action == VFIO_IOMMU_NOTIFY_DMA_UNMAP) {
		...
		vfio_unpin_pages(matrix_mdev->vdev.dev, &g_pfn, 1);
		return NOTIFY_OK;
	}
